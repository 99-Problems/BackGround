...
namespace Coffee.UIExtensions
{
...
#endif
    {
        ...
        private void CheckUseableShader()
        {
#if UNITY_EDITOR
            foreach (var mit in particles)
            {
                var renderer = mit.GetComponent<ParticleSystemRenderer>();
                if (renderer)
                {
                    if (renderer.sharedMaterial?.shader?.name == "Universal Render Pipeline/Particles/Unlit_HDR")
                    {
                        Debug.LogError($"{name} : UI파티클에는 Unlit_Hdr쎄이더를 사용할 수 없습니다");
                    }
                }
            }
#endif
        ...
        private Material GetModifiedMaterial(Material baseMaterial, Texture2D texture)
        {
            if (0 < m_StencilValue)
            {
                baseMaterial = StencilMaterial.Add(baseMaterial, (1 << m_StencilValue) - 1, StencilOp.Keep, CompareFunction.Equal, ColorWriteMask.All,
        ...
        private void InitializeIfNeeded()
        {
            if (enabled && m_IsTrail)
            {
                UnityEngine.Debug.LogWarningFormat(this,
                    "[UIParticle] The UIParticle component should be removed: {0}\nReason: UIParticle for trails is no longer needed.", name);
