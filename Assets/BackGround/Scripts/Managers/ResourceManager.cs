...
public class ResourceManager : MonoBehaviour
{
    ...
    public static string BGAssetPath = "Assets/BackGround/Prefabs/";
    ...
    {
        var loadedObj = await LoadAsync<GameObject>(bundleName, ZString.Concat(assetName, ".prefab"));
        if (loadedObj == null)
        {
    ...
    public partial class ObjectPool
    {
        ...
        public async UniTask Load<T>(string bundleName, string assetName, Action<T> callback, int lifeCount, Define.AssetLabel label = Define.AssetLabel.Default)
            where T : Object
        {
            string key = ZString.Concat(bundleName, "/", assetName);
        ...
        {
            string key = ZString.Concat(dirName, "/", assetName);
            if (items.TryGetValue(key, out ObjectItem ret))
            {
                ret.lifeCount = lifeCount;
                return ret.obj as T;
            ...
            var assetPath = ZString.Concat(BGAssetPath, key);
        ...
        public T LoadDefaultAssetOnEditor<T>(string dirName, string assetName, int lifeCount) where T : Object
        {
            ...
            var newObj = AssetDatabase.LoadAssetAtPath<T>(ZString.Concat(BGAssetPath, dirName, "/", assetName));
            if (newObj != null)
            {
                if (lifeCount > 0)
                {
