    ...
    static void Init()
    {
        if(S_Instance == null)
        {
            ...
            if (go == null)
            {
                ...
                //go.AddComponent<SpriteAtlasManagerEx>();
            }
...
#if UNITY_EDITOR
    private const string drawGizmos = "Menu/DrawGizmo";
    public static bool isDrawGizmos => EditorPrefs.GetBool(drawGizmos);

    [MenuItem(drawGizmos)]
    private static void DrawGizmosToggle()
    {
        var _isDrawGizmos = !isDrawGizmos;
        Menu.SetChecked(drawGizmos, _isDrawGizmos);
        EditorPrefs.SetBool(drawGizmos, _isDrawGizmos);
